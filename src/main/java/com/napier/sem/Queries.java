...
///the one am working on right now... it should basically allow the user to create all the other missing queries...
    ...
    public static void Statement4() throws SQLException {
        ...
        try(Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/world", "root", "BkQR7Aczt")) {
            Statement stmt = con.createStatement();
            Scanner scanner = new Scanner(System.in);
            //get the contient from the user
            ...
            while (rs.next()) {
                ...
                //if the contient is the contient choosen from the user then display info
    ...
    public static void Statement2() throws SQLException {
        ...
        try(Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/world", "root", "BkQR7Aczt")) {
            ...
            while (rs.next()) {
                ...
                int population = rs.getInt("Population");
                String continent = rs.getString("Continent");
                //if the contient matches the choosen contient display info
